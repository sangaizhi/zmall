                    ...
                    <section class="panel">
                        <div class="panel-body">
                            <div class="adv-table editable-table ">
                                <div class="clearfix">
                                    <div class="btn-group">
                                        <a href="../role/add.html" id="editable-sample_new" class="btn btn-primary">
                                            新增
                                        </a>
                                    </div>
                                        ...
                                        <c:if test="${list.size() > 0}">
                                            <c:forEach items="${list}" var="role">
                                                <tr>
                                                    <td></td>
                                                    <td>${role.name}</td>
                                                    <td>${null != role.createUser ? role.createUser.name : ''}</td>
                                                    <td><fmt:formatDate value="${role.createTime}"
                                                                        pattern="yyyy-MM-dd hh:mm:ss"/></td>
                                                    <td>${null != role.modifyUser ? role.modifyUser.name : ''}</td>
                                                    <td><fmt:formatDate value="${role.lastModifyTime}"
                                                                        pattern="yyyy-MM-dd hh:mm:ss"/></td>
                                                </tr>
                                            </c:forEach>
                                        </c:if>
